...
namespace FFT_Spectro
{
    ...
    public partial class Spec_FFT : Form
    {
        ...
        public void analise()
        {
            ...
            int limite_da_janela = cfg_dg.nzao+cfg_dg.offset;
            for (int i = cfg_dg.offset, j = 0; i < limite_da_janela; i++,j++)
            {
            ...
            // Em casos que offset!=0 o indice i vira i+offset
            double freq_para_hertz;
            double mag_para_dB;
            for (int i = 0; i < tmpComplexArray.Length; i++)
            {
                freq_para_hertz = ((double)(i + cfg_dg.offset) * (double)_reader_freq / (double)cfg_dg.nzao);
